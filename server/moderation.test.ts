...
test("admin moderation, account controls, archive recovery, and audit safety", { timeout: 60_000 }, async (t) => {
  ...
  let memberToken = memberLogin.body.token as string;
  ...
  for (const [method, route, body] of forbiddenMutations) {
    const response = await request(origin, route, { method, token: memberToken, body });
    assert.equal(response.status, 403, `${method} ${route} must reject a non-admin session`);
  }

  const missingBanReason = await request(origin, `/api/admin/users/${memberId}/moderation`, { method: "PATCH", token: adminToken, body: { action: "ban" } });
  assert.equal(missingBanReason.status, 400);
  const ban = await request(origin, `/api/admin/users/${memberId}/moderation`, { method: "PATCH", token: adminToken, body: { action: "ban", reason: "Repeated abuse" } });
  assert.equal(ban.status, 200);
  assert.equal(ban.body.user.status, "banned");
  const revokedRequest = await request(origin, `/api/user-state/${memberId}/liked-tracks`, { method: "POST", token: memberToken, body: { likedTrackIds: [] } });
  ...
  const overviewAfterBan = await request(origin, `/api/admin/overview?userId=${memberId}`, { token: adminToken });
  ...
  assert.equal((await request(origin, `/api/admin/users/${memberId}/moderation`, { method: "PATCH", token: adminToken, body: { action: "unban", reason: "Appeal approved" } })).status, 200);
  ...
  memberToken = restoredLogin.body.token;
  ...
  const duplicateProfile = await request(origin, `/api/admin/users/${memberId}/profile`, { method: "PATCH", token: adminToken, body: { username: "fixed_admin" } });
  ...
  const unconfirmedReset = await request(origin, `/api/admin/users/${memberId}/password-reset`, { method: "POST", token: adminToken, body: { newPassword: RESET_PASSWORD, confirmPassword: RESET_PASSWORD, confirmed: false } });
  assert.equal(unconfirmedReset.status, 400);
  const reset = await request(origin, `/api/admin/users/${memberId}/password-reset`, { method: "POST", token: adminToken, body: { newPassword: RESET_PASSWORD, confirmPassword: RESET_PASSWORD, confirmed: true, reason: "Account recovery" } });
  ...
  memberToken = resetLogin.body.token;

  const archived = await request(origin, `/api/admin/users/${memberId}/moderation`, { method: "PATCH", token: adminToken, body: { action: "archive", reason: "Recoverable account removal" } });
  assert.equal(archived.status, 200);
  assert.deepEqual(archived.body.cascade, { tracks: 1, playlists: 1 });
  assert.equal((await request(origin, `/api/user-state/${memberId}/liked-tracks`, { method: "POST", token: memberToken, body: { likedTrackIds: [] } })).status, 401);
  ...
  const recordOnlyRestore = await request(origin, `/api/admin/users/${memberId}/moderation`, { method: "PATCH", token: adminToken, body: { action: "restore", reason: "Review", cascade: false } });
  assert.equal(recordOnlyRestore.status, 200);
  shared = await request(origin, "/api/data?scope=shared");
  assert(!shared.body.tracks.some((track: { id: string }) => track.id === trackId));
  assert(!shared.body.playlists.some((playlist: { id: string }) => playlist.id === playlistId));

  const cascadeRestore = await request(origin, `/api/admin/users/${memberId}/moderation`, { method: "PATCH", token: adminToken, body: { action: "restore", reason: "Approved", cascade: true } });
  ...
  assert.equal((await request(origin, `/api/admin/tracks/${trackId}/archive`, { method: "PATCH", token: adminToken, body: { action: "archive", reason: "Track review" } })).status, 200);
  shared = await request(origin, "/api/data?scope=shared");
  assert(!shared.body.tracks.some((track: { id: string }) => track.id === trackId));
  assert.deepEqual(shared.body.playlists.find((playlist: { id: string }) => playlist.id === playlistId).trackIds, []);
  assert.equal((await request(origin, `/api/admin/tracks/${trackId}/archive`, { method: "PATCH", token: adminToken, body: { action: "restore", reason: "Cleared" } })).status, 200);
  assert.equal((await request(origin, `/api/admin/playlists/${playlistId}/archive`, { method: "PATCH", token: adminToken, body: { action: "archive", reason: "Playlist review" } })).status, 200);
  shared = await request(origin, "/api/data?scope=shared");
  assert(!shared.body.playlists.some((playlist: { id: string }) => playlist.id === playlistId));
  assert.equal((await request(origin, `/api/admin/playlists/${playlistId}/archive`, { method: "PATCH", token: adminToken, body: { action: "restore", reason: "Cleared" } })).status, 200);

  const protectedAdmin = await request(origin, `/api/admin/users/${ADMIN_USER_ID}/moderation`, { method: "PATCH", token: adminToken, body: { action: "ban", reason: "Must be rejected" } });
  assert.equal(protectedAdmin.status, 400);

  const finalOverview = await request(origin, `/api/admin/overview?userId=${memberId}`, { token: adminToken });
