...
export const PlaylistView: React.FC<PlaylistViewProps> = ({
  ...
  const mutateTrack = async (track: Track, action: 'add' | 'remove') => {
    ...
    try {
      ...
      if (succeeded) showToast?.(t(action === 'add' ? 'Added "{{track}}" to {{playlist}}' : 'Removed "{{track}}" from {{playlist}}', { track: track.title, playlist: playlist.title }));
    } finally {
  ...
  const handleCopyLink = async () => {
    ...
    showToast?.(t(copied ? 'Playlist link copied.' : 'Could not copy the playlist link.'));
  };
    ...
    <div className="workspace-screen min-h-full space-y-6 overflow-x-hidden pb-16 text-white select-none">
      <section data-playlist-id={playlist.id} data-context-type="playlist" className="workspace-card section-reveal relative overflow-hidden rounded-[2rem] border border-white/10 bg-[#17131d] shadow-2xl">
        ...
        <div className="relative flex flex-col gap-6 p-5 sm:p-7 md:flex-row md:items-end md:p-9">
          ...
          <div className="min-w-0 flex-1 text-center md:text-left">
            ...
            <p className="mx-auto mt-3 max-w-2xl text-sm leading-6 text-zinc-400 md:mx-0">{playlist.description || t('A personal mix from your VERTEX Music library.')}</p>
            <div className="mt-4 flex flex-wrap items-center justify-center gap-3 text-[11px] font-bold text-zinc-400 md:justify-start">
              <span className="flex items-center gap-1.5 text-white"><ListMusic className="h-3.5 w-3.5 text-[#D946EF]" /> {t(playlistTracks.length === 1 ? '{{count}} track' : '{{count}} tracks', { count: playlistTracks.length })}</span>
      ...
      <section className="workspace-card section-reveal flex flex-col items-stretch gap-4 rounded-3xl border border-white/10 bg-[#181818] p-4 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between sm:p-5">
        <div className="flex flex-wrap items-center justify-center gap-2.5 sm:justify-start">
          ...
          <button type="button" onClick={() => onShufflePlaylist(playlistTracks)} disabled={playlistTracks.length === 0} className={`control-press flex h-11 items-center gap-2 rounded-2xl border px-4 text-xs font-black transition-all disabled:opacity-40 ${isCurrentPlaylistShuffled ? 'border-[#D946EF]/50 bg-[#D946EF]/15 text-[#F0ABFC]' : 'border-white/10 bg-white/5 text-zinc-300 hover:bg-white/10 hover:text-white'}`} title={t(isCurrentPlaylistShuffled ? 'Turn shuffle off' : 'Shuffle playlist')}>
            <Shuffle className="h-4 w-4" /> <span className="hidden sm:inline">{t(isCurrentPlaylistShuffled ? 'Shuffling' : 'Shuffle')}</span>
          </button>
      ...
      <section className="workspace-card section-reveal overflow-hidden rounded-3xl border border-white/10 bg-[#181818]">
        <div className="flex items-end justify-between gap-4 border-b border-white/[0.08] px-4 py-4 sm:px-6">
          ...
          <span className="rounded-full border border-white/[0.08] bg-black/20 px-3 py-1.5 text-[10px] font-bold text-zinc-500">{t(playlistTracks.length === 1 ? '{{count}} song' : '{{count}} songs', { count: playlistTracks.length })}</span>
        </div>
      ...
      {canManage && showAddSection && createPortal(
        ...
        <section role="dialog" aria-modal="true" aria-label={t('Add songs to {{title}}', { title: playlist.title })} className="workspace-card section-reveal flex max-h-[min(82dvh,760px)] w-full max-w-4xl flex-col overflow-hidden rounded-[2rem] border border-[#D946EF]/20 bg-gradient-to-b from-[#211526] to-[#181818] shadow-[0_32px_100px_rgba(0,0,0,0.9)]">
          ...
          {availableTracks.length === 0 ? (
            <div className="rounded-2xl border border-white/[0.06] bg-black/15 p-8 text-center text-xs text-zinc-500">{t(normalizedQuery ? 'No available tracks match this search.' : 'Every catalog track is already in this playlist.')}</div>
          ) : (
