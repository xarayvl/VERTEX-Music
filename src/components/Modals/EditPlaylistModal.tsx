        ...
        <header className="relative flex min-w-0 items-start justify-between gap-3 border-b border-white/10 px-4 py-4 sm:gap-5 sm:px-7 sm:py-5">
          <div className="flex min-w-0 items-center gap-3 sm:gap-4">
            ...
            <div className="min-w-0">
              <div className="mb-1 flex items-center gap-1.5 text-[8px] font-black uppercase tracking-[0.14em] text-[#D8B4FE] sm:gap-2 sm:text-[9px] sm:tracking-[0.22em]"><Sparkles className="h-3.5 w-3.5" /> {t('Playlist workspace')}</div>
              <h2 className="text-xl font-black tracking-tight sm:text-2xl">{t('Edit playlist')}</h2>
              <p className="mt-1 text-xs text-zinc-500">{t('Update its identity without changing the track order.')}</p>
            </div>
          </div>
          <button
            type="button"
            ...
            aria-label={t('Close playlist editor')}
          >
            <X className="h-5 w-5" />
          </button>
        </header>
        ...
        <form onSubmit={handleSubmit} className="relative grid min-w-0 gap-5 p-4 sm:gap-6 sm:p-7 lg:grid-cols-[0.9fr_1.1fr]">
          <section className="rounded-3xl border border-white/10 bg-gradient-to-b from-[#24182d] to-[#181818] p-4 sm:p-5">
            <div className="group relative mx-auto aspect-square w-full max-w-sm overflow-hidden rounded-[1.6rem] border border-white/10 bg-black shadow-2xl">
              ...
              <button type="button" onClick={() => coverFileInputRef.current?.click()} className="absolute inset-0 flex flex-col items-center justify-center gap-2 bg-black/55 opacity-0 transition-opacity group-hover:opacity-100 focus:opacity-100">
                ...
                <span className="text-xs font-black">{t('Upload new cover')}</span>
              </button>
              <div className="pointer-events-none absolute inset-x-0 bottom-0 p-5">
                <p className="text-[9px] font-black uppercase tracking-[0.2em] text-[#E9D5FF]">{t('Live preview')}</p>
          ...
          <section className="rounded-3xl border border-white/10 bg-[#181818] p-5 sm:p-6">
            <div className="space-y-5">
              <div><label className={labelClass}>{t('Playlist title')}</label>
                <input
                  type="text"
                  required
                  autoFocus
              ...
              <div><div className="mb-2 flex items-center justify-between"><label className="text-[11px] font-black uppercase tracking-[0.16em] text-zinc-400">{t('Description')}</label><span className="text-[9px] font-mono text-zinc-600">{description.length}/1000</span></div>
                <textarea
                  value={description}
              ...
              <div><label className={`${labelClass} flex items-center gap-2`}><ImageIcon className="h-4 w-4 text-[#D946EF]" /> {t('Cover URL')}</label><input type="text" value={coverUrl.startsWith('data:') ? '' : coverUrl} onChange={(e) => { setCoverUrl(e.target.value); setCoverFileName(''); setErrorMessage(''); }} placeholder={t(coverUrl.startsWith('data:') ? 'Uploaded image selected' : 'Paste an HTTP(S) image URL')} className={fieldClass} /></div>
            </div>
            ...
            <div className="mt-7 flex flex-col-reverse gap-3 border-t border-white/10 pt-5 sm:flex-row sm:justify-end">
              <button type="button" onClick={onClose} disabled={isSaving} className="control-press rounded-2xl border border-white/10 bg-white/5 px-5 py-3 text-sm font-bold text-zinc-300 hover:bg-white/10 hover:text-white disabled:opacity-40">{t('Cancel')}</button>
              <button type="submit" disabled={!title.trim() || isSaving || savedSuccess} className={`control-press flex items-center justify-center gap-2 rounded-2xl px-6 py-3 text-sm font-black shadow-[0_14px_36px_rgba(168,85,247,0.24)] disabled:cursor-not-allowed disabled:opacity-60 ${savedSuccess ? 'bg-emerald-500 text-white' : 'bg-gradient-to-r from-[#A855F7] to-[#D946EF] hover:brightness-110'}`}>
                {savedSuccess ? <><Check className="h-4 w-4" /> {t('Saved')}</> : isSaving ? <><Loader2 className="h-4 w-4 animate-spin" /> {t('Saving...')}</> : <><Save className="h-4 w-4" /> {t('Save changes')}</>}
              </button>
            </div>
          </section>
        </form>
